...
def build_matchup_features(
    ...
    # ── Clutch performance (last 20 games, close-game win rate) ──────────────
    ...
    # Positive = home team forces faster pace than away prefers, relative to defense
    matchup["pace_scheme_advantage"] = (
        (home_feats["pace_last10"] - away_feats["pace_last10"])
        * (away_feats["def_rtg_last10"] - home_feats["def_rtg_last10"])
        / 100.0
    )
...
def _clutch_win_pct(
    ...
    Uses final score margin only — we don't have play-by-play close-game data.
    Close wins reveal composure under pressure more than blowout wins.
    """
...
def _opp_adjusted_ratings(
    ...
    Scales each game's ortg/drtg by opponent Elo relative to league average (1500),
    so strong performances vs elite opponents count more than vs weak opponents.
    """
    defaults = {"adj_ortg": 110.0, "adj_drtg": 110.0}
    try:
        result = session.execute(
            text("""
                SELECT
        ...
        return {
            "adj_ortg": float(sum(adj_ortg_vals) / len(adj_ortg_vals)) if adj_ortg_vals else 110.0,
            "adj_drtg": float(sum(adj_drtg_vals) / len(adj_drtg_vals)) if adj_drtg_vals else 110.0,
        }
    except Exception:
        return defaults
